...
#### P0 (필수) ✅ 완료
- [x] `IssueForm` - 이슈 생성/수정 폼
- [x] `ProjectForm` - 프로젝트 생성/수정 폼
- [x] 팀 관리 컴포넌트
- [x] 이슈/프로젝트 삭제 UI
...
- [x] 전역 검색 기능
- [x] 사용자 프로필 관리
- [x] `NotificationList` - 알림 목록 (NotificationDropdown 완료)
- [x] 실시간 알림 업데이트 (폴링)
- [ ] 고급 검색/필터 UI (기본 기능은 완료, UX 개선) - 선택사항
...
> **기준**: PRD.md 및 현재 개발 상태 분석

### 13.1 P1 (권장) - 향상 기능

#### 1. 전역 검색 기능 (3.2.1) ⚠️ **우선순위 높음**
...
**현재 상태:**
- ✅ 페이지별 검색: FilterBar로 구현됨 (이슈, 프로젝트, 팀)
- ❌ 전역 검색: 미구현
- ❌ 검색 결과 페이지: 미구현
- ❌ 검색 히스토리: 미구현

**제안 구현:**
1. **전역 검색 컴포넌트** (`src/components/search/GlobalSearch.tsx`)
   - Header에 검색 아이콘/입력 필드 추가
   - 키보드 단축키 지원 (Cmd/Ctrl + K)
   - 검색 모달/드롭다운
...
#### 2. 사용자 프로필 관리 (3.2.2) ⚠️ **우선순위 높음**

**PRD 요구사항:**
- 로그인/로그아웃 기능 ✅ 완료
- 사용자 프로필 관리 ❌ 미구현
- 권한 관리 (필요시) - 팀 관리에서 부분적으로 구현됨

**현재 상태:**
- ✅ 로그인/로그아웃: 완료
- ✅ Header에 프로필 드롭다운: 완료 (프로필 클릭 이벤트 있음)
- ❌ 프로필 페이지: 미구현 (`/profile` 또는 `/settings/profile`)
- ❌ 프로필 수정 폼: 미구현
- ❌ 비밀번호 변경: 미구현
- ❌ 아바타 업로드: FileUpload 컴포넌트는 있지만 프로필에 연결 안됨

**제안 구현:**
1. **프로필 페이지** (`/profile` 또는 `/settings/profile`)
   - 사용자 정보 표시 (이름, 이메일, 아바타)
   - 프로필 수정 폼
   - 비밀번호 변경 폼
   - 아바타 업로드 (FileUpload 컴포넌트 활용)
2. **프로필 수정 API 연동**
   - `useAuthStore`에 `updateProfile` 메서드 추가
   - 또는 별도 `useProfile` 훅 생성
3. **Header 프로필 클릭 연결**
   - `onProfileClick` 핸들러에서 `/profile`로 라우팅

**예상 시간**: 2-3시간

### 13.2 추가로 확인/개선이 필요한 기능

#### 3. 이슈/프로젝트 삭제 UI

**현재 상태:**
- ✅ 삭제 로직: `useIssue.deleteIssue`, `useProject.deleteProject` 존재
- ❓ 삭제 UI: 페이지에서 삭제 버튼/액션 확인 필요

**확인 필요:**
- `/issues/[id]` 페이지에 삭제 버튼 있는지
- `/projects/[id]` 페이지에 삭제 버튼 있는지
- 삭제 확인 다이얼로그 있는지

**제안:**
- 삭제 버튼 추가 (Dropdown 메뉴 또는 별도 버튼)
- 삭제 확인 모달/다이얼로그 추가
- 삭제 후 목록 페이지로 리다이렉트
...
**현재 상태:**
- ✅ FileUpload 컴포넌트: 존재 (`src/components/forms/FileUpload.tsx`)
- ❌ 이슈에 파일 첨부 기능: 미구현
- ❌ IssueForm에 파일 업로드 필드: 없음

**PRD 명시 여부:**
- PRD에 명시되지 않았지만 Issue Tracking 시스템으로서 일반적인 기능

**제안:**
- IssueForm에 파일 첨부 필드 추가
- 이슈 상세 페이지에서 첨부파일 목록 표시
- 파일 다운로드 기능

**예상 시간**: 2-3시간 (P2로 분류 가능)

#### 5. 실시간 알림 업데이트

**현재 상태:**
- ✅ 알림 시스템: NotificationDropdown 완료
- ✅ 알림 훅: `useNotifications` 완료
- ❓ 실시간 업데이트: 폴링 또는 WebSocket 구현 여부 확인 필요

**제안:**
- 폴링 기반 실시간 업데이트 (간단)
- 또는 WebSocket 연동 (복잡, API 필요)
...
#### 즉시 구현 (P1 - 권장)
1. **전역 검색 기능** (2-3시간) - 사용자 경험 향상, PRD 명시 기능
2. **사용자 프로필 관리** (2-3시간) - PRD 명시 기능, Header 프로필 클릭 연결 필요

#### 단기 개선 (확인 후 진행)
3. **이슈/프로젝트 삭제 UI** (1시간) - 로직은 있지만 UI 확인 필요
4. **실시간 알림 업데이트** (1-2시간) - 폴링 기반으로 간단히 구현 가능

#### 중기 개선 (P2 - 선택)
5. **파일 업로드 기능** (2-3시간) - PRD에 명시되지 않음, Issue Tracking 시스템으로서 유용

### 13.4 추천 진행 순서

1. **전역 검색 기능** (P1, 2-3시간) - 가장 사용 빈도가 높은 기능
2. **사용자 프로필 관리** (P1, 2-3시간) - Header 프로필 클릭 연결 필요
3. **이슈/프로젝트 삭제 UI 확인 및 추가** (1시간) - 빠르게 완료 가능
4. **실시간 알림 업데이트** (1-2시간) - 사용자 경험 개선
5. **파일 업로드 기능** (P2, 2-3시간) - 선택적 기능
