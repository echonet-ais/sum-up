      ...
      <DrawerContent maxWidth="sm" align="center" className="px-4 sm:px-6">
        {loading ? (
          <div className="space-y-4 sm:space-y-6">
            {[1, 2].map((i) => (
              <div key={i} className="micro-card rounded-lg border border-[var(--border-subtle)] bg-[var(--surface)] p-6">
          ...
          <div className="flex flex-wrap gap-2 sm:gap-3 w-full">
            {actions.map((action, index) => (
              <Link
                key={index}
                href={action.href}
                className="micro-button inline-flex items-center gap-2 rounded-lg border border-[var(--border-subtle)] bg-[var(--surface)] px-3 sm:px-4 py-2 text-xs sm:text-sm font-medium text-[var(--text-strong)] transition-all hover:border-[var(--brand-primary)] hover:text-[var(--brand-primary)]"
                onClick={() => {
                  action.onClick?.();
                  onOpenChange(false);
                }}
              >
                {action.icon && <Icon name={action.icon as "externalLink" | "list" | "arrowRight"} size={14} className="sm:w-4 sm:h-4" provider="phosphor" />}
                {action.label}
              </Link>
            ))}
          </div>
